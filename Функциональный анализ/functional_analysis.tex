...
\end{enumerate}
 ...
 Пусть есть совокупность линейных непрерывных операторов $A_{\alpha}$ на гильбертовом пространстве $H$, которая не ограничена по норме. Покажем, что есть вектор, на котором она неограничена.
 Построим по индукции последовательности $A_n\in \{A_{\alpha}\}$, $f_n\in H$ с такими свойствами:
 \begin{enumerate}
 \item $\|f_n\| < \frac{1}{2^n (1 + \max(\|A_1\|, \ldots, \|A_{n-1}\|))}$;
 \item $\|A_n f_n\| > 2^n + \|A_n(f_1+\ldots+f_{n-1})\|.$
 \end{enumerate}

 Имеем:
 \begin{enumerate}
 \item Ряд $\sum\limits_n{f_n}$ сходится, ибо $\|f_n\|<\frac{1}{2^n}$. Пусть $f=\sum\limits_n {f_n}.$
 \item При $k>0$ имеем
 $$\|A_n f_{n+k}\| < \frac{1}{2^{n+k}}.$$
 Значит, для любого $n$ ряд
 $$\sum\limits_{k} \|A_n f_k\|$$
 сходится.
 Отсюда получаем, что
 $$A_nf = A_n(f_1 + \ldots + f_n) + A_n\sum\limits_{k=n+1}^{\infty} f_k,$$
 $$\|A_n(f_1+\ldots+f_n)\|>2^n, \|A_n\sum\limits_{k=n+1}^{\infty} f_k\|<\frac{1}{2^n}.$$
 В итоге, получаем
 $$\|A_nf\|>2^n-\frac{1}{2^n}.$$
