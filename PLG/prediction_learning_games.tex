...
Имеем
 ...
 Всё ясно.
 \item
 \item Воспользуемся указанием. Надо просто обратить неравенства в доказательстве теоремы 2.2.
 Для первого неравенства имеем
$$\ln\frac{W_n}{W_0}\le \ln\left(N \max\limits_{1\le i \le N}e^{-\eta L_{i, n}}\right)-\ln N=-\eta\min\limits_{i=1,\ldots, N}L_{i, n}.$$
Второе неравенство доказывается сложнее.
Имеем
$$\ln\frac{W_i}{W_{i-1}}=\ln\frac{\sum\limits_{i=1}^N w_{i, t-1}e^{-\eta|f_{i,t}-y_t|}}{\sum\limits_{j=1}^N w_{j, t-1}}\ge
\ln e^{-\eta|\hat{p}_t-y_t|}=-\eta l(\hat{p}_t, y_t).$$
Неравенство написано в силу выпуклости функции
$$f(x)=e^{-\eta|x-y|}$$
на $D=[0, 1]$ как при $y=0$, так и при $y=1$ (а у нас всегда $y_t=0$ или $y_t=1$ по условию).
Ну и всё. Эти неравенства суммируем, получаем
$$\ln\frac{W_n}{W_0} \ge -\eta \hat{L}_n.$$
И сопоставляя с первым неравенством, получаем
$$\hat{L}_n \ge \min_{i=1,\ldots, N}L_{i, n}.$$
